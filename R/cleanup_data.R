...
cleanup_data.data.frame = function(data, ...) {
  ...
  if (nc == 2) {
    ...
    if (max(table(data$minute)) > 1) {
      stop(
        "Only two columns, and multiple PDR values for given time. Forgot column pat_id?"
      )
    }
  }
  ...
  if (nc == 4) {
    ...
    if (max(table(data$minute, data$patient_id, data$group)) > 1) {
      warning(
        "Multiple data for one patient, minute and group. Included the same patient's data twice?"
      )
    }
  ...
  if (!has_group) {
    ...
    if (!all(with(data, table(patient_id, minute)) %in% 0:1)) {
      stop(
        "Multiple values for the same patient at the same minute require a <<group>> column"
      )
    }
    data$group = "A"
  } else {
...
cleanup_data.matrix = function(data, ...) {
  if (ncol(data) > 2) {
    stop(
      "A matrix can only be used as data input when two columns <minute> and <pdr> are passed. Use a data frame otherwise"
    )
  }
...
cleanup_data.list = function(data, ...) {
  ...
  ret = tibble::as_tibble(
    ret[, c("patient_id", "group", "minute", "pdr")],
    .name_repair = "minimal"
  )
