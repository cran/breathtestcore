...
#' @param linewidth optional line width; can improve look for printouts
...
plot.breathtestfit = function(
  ...
  if (has_groups) {
    ...
    if (has_fit) {
      p = p +
        geom_line(
          aes(x = minute, y = fitted, color = group),
          linewidth = linewidth,
          data = dd,
          show.legend = FALSE
        ) +
  ...
  if (is(x, "breathtestnlsfit")) {
    fit = "Individual curve fit (nls)."
  } else if (is(x, "breathtestnlmefit")) {
    fit = "Population fit (nlme)."
  } else if (is(x, "breathteststanfit")) {
    fit = "Bayesian fit (Stan)."
  } else {
    fit = ""
  }
  subtitle = ifelse(
