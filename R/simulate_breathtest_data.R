...
simulate_breathtest_data = function(
  ...
  if (use_cov && is.null(cov)) {
    cov =
      structure(
        ...
        .Dim = c(3L, 3L),
        .Dimnames = list(c("m", "k", "beta"), c("m", "k", "beta"))
      )
  }
  ...
  sf = ifelse(
    is.null(student_t_df) || student_t_df == 0,
    "Gaussian",
  ...
  if (!use_cov) {
    ...
    comment = sprintf(
      "%.0f records,  m = %.0f\U00B1%.0f, k =  %.3f\U00B1%.3f, %s,\n%s noise amplitude = %.0f, %.0f%% missing",
      n_records,
      m_mean,
      m_std,
      k_mean,
    ...
    comment = sprintf(
      "%.0f records,  m = %.0f, k =  %.3f, beta =  %.1f, cov-matrix, 
 %s noise amplitude = %.0f, %.0f%% missing",
      n_records,
      m_mean,
      k_mean,
