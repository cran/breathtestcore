...
read_iris_csv = function(filename = NULL, text = NULL) {
  ...
  if (
    str_detect(header, ("^\"Name\",\"Vorname\",\"Test\",\"Identifikation\"")) !=
      1
  ) {
    stop(paste(
      "File",
  ...
  d = suppressWarnings(readr::read_csv(
    to_read,
    col_types = "cccciddddddcc",
  ...
  if (ncol(d) != 13) {
    stop(paste(
      "File in IRIS CSV format",
      filename,
      "has unexpected",
      ncol(d),
      "columns; expected 13 columns"
    ))
  }
  if (nrow(d) < 5) {
    stop(paste(
  ...
  if (nchar(name) > 0 && nchar(first_name) > 0) {
    initials = paste0(str_sub(name, 1, 1), str_sub(first_name, 1, 1))
  }
