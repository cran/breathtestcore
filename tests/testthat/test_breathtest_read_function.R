...
check_and_read = function(filename, read_function) {
  ...
  if (inherits(bt, "breathtest_data_list")) {
    expect_true(all(map_lgl(bt, function(x) {
      class(x) == "breathtest_data"
    })))
    bt = bt[[1]]
...
test_that("Valid files with datea errors throw exception", {
  expect_error(
    read_any_file("IrisCSV_MissingColumn.TXT"),
    "unexpected 12 columns"
  )
  expect_error(
    read_any_file("IrisCSV_invalidValues.TXT"),
    "Invalid or missing PDR/DOB data"
  )
})
